  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      CREATE TABLE task.tasks (
        ...
        parent_task_id  uuid,
        -- Derived on insert, kept as a column so depth limits and "root tasks
        ...
        sprint_id       uuid,
        estimate        numeric,
        -- Keyed by field UUID, so a rename does not orphan every value. (Ph 4)
        ...
        CONSTRAINT tasks_depth_non_negative_check
          CHECK (depth >= 0),
        -- MAX_TASK_DEPTH in @repo/shared: 1 means a task and a sub-task.
        -- Spelled out rather than imported — a migration that changes when
        -- someone edits a shared file is not a record. A test holds them
        -- together.
        ...
        -- RESTRICT: emptying a project or status is a deliberate act.
    ...
    `)
    // A GIN index on custom_fields waits for Phase 4 to query it.
