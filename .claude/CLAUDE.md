...
- `route-metadata.ts` — `@Public()` and `@SkipOrgScope()`, together because they are easy to confuse: the first says no sign-in is needed, the second says signed in but bound to no org. Nothing reads either until Phase 1's guard.
