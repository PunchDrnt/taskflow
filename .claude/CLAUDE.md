...
- `request-context.ts` — `AsyncLocalStorage<{ orgId, userId }>`. `requireRequestContext()` throws rather than defaulting; there is no safe guess for an org. `runWithRequestContext` scopes to a callback and is what jobs, seeds and tests use; `enterRequestContext` wraps `enterWith` and has no closing boundary, which is why it exists only for the guard.
...
**`http/`** — what every request passes through on its way in and out.

- `route-metadata.ts` — `@Public()` and `@SkipOrgScope()`, together because they are easy to confuse: the first says no sign-in is needed, the second says signed in but bound to no org. Read by Phase 1's `AuthGuard`, which is what it moved here to sit beside.
- `api-exception.ts` — `ApiException`, an error carrying the `code` the client branches on. Extends `HttpException` rather than `Error` because `SentryGlobalFilter` reports anything that is not one as unhandled — a plain `Error` subclass would page someone for every bad email address.
- `api-exception.filter.ts` — renders `{ statusCode, code, message, details? }` from `docs/01-architecture.md#api`. `@Catch(HttpException)`, deliberately narrow: everything else falls through to `SentryGlobalFilter`, which is the only way it still gets reported. The price is that a 500 body carries no `code`. The two are registered in `app.module.ts` in an order that matters — `APP_FILTER` applies in reverse, last registered first — and only Nest's own exceptions depend on it, since an `ApiException` carries its finished body as the `HttpException` payload and renders the same either way.
- `zod-validation.pipe.ts` — `@Body(new ZodValidationPipe(schema))`, per route rather than global, so the schema is visible at the route it guards. It converts the `ZodError` into an `ApiException` for the Sentry reason above, and hands the handler the _parsed_ value, so an email arrives trimmed and lower-cased.

At the root, belonging to none of the four: `shared.module.ts`, plus
