...
| **System role**        | สิทธิ์ระดับทั้งเว็บ (`iam.roles`) · พวกเราดูแล                                         | ไม่ใช่ role ใน org          |
...
| `org_id` ทุกตารางที่แถวเป็นของ org ใด org หนึ่ง (ยกเว้น `iam`, `billing.plans`, `organizations` ที่ไม่เป็นของ org ไหน · และ `notify.outbox` ที่ **nullable** เพราะเมลรีเซ็ตรหัสผ่านเป็นของ *บัญชี* ไม่ใช่ของ org — [เหตุผล](./02-database/schema.md#schema-notify)) + ต้องมี isolation test | ข้อมูลข้ามบริษัทรั่ว — พลาดครั้งเดียวจบ                                    | [`02-database/rules.md`](./02-database/rules.md#multi-tenancy)                                           |
