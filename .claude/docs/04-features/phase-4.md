...
## Group by — ย้ายไปฝั่ง server

Phase 1 จัดกลุ่ม **ฝั่ง client จากแถวที่โหลดมาแล้ว** ปุ่มโหลดเพิ่มมีปุ่มเดียวใต้ลิสต์
เลขข้างหัวกลุ่มจึงแปลว่า "โหลดมาเท่านี้" ไม่ใช่ยอดจริง — เป็นผลตรงๆ ที่รับไว้ตั้งแต่ต้น
เพราะทางเลือกคือ endpoint ต่อหนึ่งวิธีจัดกลุ่ม ซึ่งไม่คุ้มกับ dataset ขนาดนี้

ที่พอใช้ได้ตอนนี้เพราะของน้อย: ขอ `limit` เต็มเพดาน (100) แล้วไล่ cursor ต่อจนหมด
ประมาณ 300 แถวก็ครอบคลุมเกือบทุกโปรเจกต์ของบริษัทนี้ — ข้อมูลอยู่ในมือครบ กลุ่มก็นับถูก
โดยไม่ต้องแตะ API เลย **จุดที่ต้องย้ายคือตอนโปรเจกต์เดียวทะลุเพดานนั้นจริง** ไม่ใช่ก่อนหน้านั้น

| เครื่องมือ         | วิธี                                                                           |
| ------------------ | ------------------------------------------------------------------------------ |
| **Notion / Asana** | กลุ่ม = query ของตัวเอง มี cursor ของตัวเอง ปุ่มโหลดเพิ่ม**อยู่ในกลุ่ม** ยอดข้างชื่อกลุ่มเป็นยอดจริง |
| **Linear**         | sync ทั้ง workspace ลง IndexedDB แล้ว group / filter / นับ ในเครื่องทั้งหมด ไม่มี paging ให้เห็น  |

Linear ไม่ใช่ทางของเรา — sync engine กับ conflict resolution เป็นโปรเจกต์ของตัวเอง
เราเดินทาง Notion และมาพร้อม view ในเฟสนี้พอดี เพราะ `group_by` ไปอยู่ใน `view.views` แล้ว

สิ่งที่ต้องมีตอนย้าย:

- **endpoint นับต่อกลุ่ม** — `GROUP BY` ครั้งเดียวด้วย filter ชุดเดียวกับลิสต์
  🔒 ห้ามยัดลง `meta` ของ `Page<T>` — `toPage` ตั้งใจดึงแถว probe แถวเดียวแทน `COUNT(*)`
  (docs/01-architecture.md#api) ยอดต่อกลุ่มเป็นคำถามคนละคำถาม ต้องเป็น response ของตัวเอง
- **cursor ต่อกลุ่ม** — หนึ่งกลุ่ม = filter หนึ่งค่าที่ API รับอยู่แล้ว (`statusId` `priority`
  `assigneeId` และ due bucket ที่แปลงเป็น `dueAfter`/`dueBefore`) เปิดหน้าหนึ่งจึงเท่ากับ N request
  และ N cursor — นั่นคือราคาจริงของยอดที่ถูกต้อง
- **รายชื่อกลุ่มมาจากนิยาม ไม่ใช่จากแถว** — status ทั้งบอร์ด, priority ทั้งสี่ค่า
  กลุ่มว่างถึงจะมีตัวตนให้เลือกซ่อนหรือแสดงได้ ซึ่งเป็น setting ระดับ view
- ⚠️ **อย่ารวม `GROUP BY` กับ keyset cursor ในคำสั่งเดียว** — cursor เรียง *แถว*
  แต่ผลที่จัดกลุ่มแล้วเรียง *ถัง* ไม่มีตำแหน่งให้ cursor อ้างถึง
