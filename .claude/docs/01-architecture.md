...
| | member | admin | owner |
| --- | --- | --- | --- |
| อ่านข้อมูล org (คน · role · ตั้งค่า) | ✓ | ✓ | ✓ |
| อ่าน project / task | เฉพาะที่เป็นสมาชิก | ✓ | ✓ |
| สร้าง project | | ✓ | ✓ |
...
> ⚠️ **สองแถวแรกเคยเขียนรวมกันว่า "อ่านทุกอย่างใน org ✓✓✓" และแถว "สร้าง project" เคยติ๊ก member ด้วย
> — ทั้งคู่ไม่ตรงกับ `ability.ts`** · แก้ตอนทำหน้า Organization settings ซึ่งเอาตารางนี้ขึ้นจอจริง
> · member ได้ `can('read','all')` แล้วถูก `cannot('read','Project')` / `cannot('read','Task')` เอาคืน
>   แล้วคืนให้เป็นราย project ที่เป็นสมาชิก ([สิทธิ์ระดับ project](./04-features/phase-1.md#สิทธิ์ระดับ-project-กั้นจริงตั้งแต่-phase-1))
> · "สร้าง project ได้เฉพาะ owner/admin" อยู่ใน [phase-1.md#project](./04-features/phase-1.md#project) มาตั้งแต่ต้น
>   — โค้ดแก้ตามไปแล้ว ตารางนี้เป็นที่สุดท้ายที่ยังเขียนของเก่าไว้
...
| counter ที่เลือกอยู่บนหน้า **Home** | `useState` ใน `HomeWork` | **ข้อยกเว้นเดียวของแถวแรก และตั้งใจ** — "งานที่เลยกำหนดของฉัน" ส่งลิงก์ให้ใครไม่ได้อยู่แล้ว เพราะปลายทางเห็นของตัวเอง · เอาเข้า URL = ได้ลิงก์ที่เปิดแล้วว่างเปล่า · ทั้งสามกองมาจาก request เดียวกันที่โหลดไว้แล้ว ไม่ได้ยิงใหม่ตอนกด |
