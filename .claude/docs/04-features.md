...
- Docker compose ครบ (api, web, postgres, postgres-test, minio, caddy) — ตอนนี้มีแค่ postgres + postgres-test
...
- Base entity — UUID, `org_id`, `created_at/by`, `updated_at/by`, `deleted_at/by` · วันเวลาใช้ `timestamptz` ทั้งหมด ([schema เต็ม](./02-database.md#base-entity--on-every-table-no-exceptions))
